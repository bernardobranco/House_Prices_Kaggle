...
print("Train label size:",label_df.shape)

################################################################################

def rmse_cv(model):
    rmse= np.sqrt(-cross_val_score(model, train_df_munged, y_train, scoring="neg_mean_squared_error", cv = 5))
    return rmse
...
kr.fit(train_df_munged, label_df)
print('Kernel ridge (exp sine squared) model obtained rmse score of: %f' % (rmse_cv(kr).mean()))
'''
################################################################################
...
y_train = np.ravel(label_df)

#alphas = np.linspace(0.0005,1,200)
#alphas = np.linspace(0.0005, 10, 100000)
#alphas = np.linspace(0.0005, 3, 1000)
#alphas = np.linspace(0.00001, 1, 10000)
#model_lasso = linear_model.LassoCV(alphas=alphas,max_iter=500000).fit(train_df_munged, y_train)
#best_alpha = 0.00099
model_lasso = linear_model.Lasso(alpha=0.0004,max_iter=500000)
model_lasso.fit(train_df_munged, y_train)
#print(model_lasso.alpha_)
print('Lasso model obtained rmse score of: %f' % (rmse_cv(model_lasso).mean()))
...
model_xgb = xgb.XGBRegressor(
                 ...
                 silent=1)
'''
model_xgb.fit(train_df_munged, label_df)

# Running a grid search on the xgboost parameters:

# Run prediction on training set to get a rough idea of how well it does.
y_pred = model_xgb.predict(train_df_munged)
y_test = label_df
#print("XGBoost score on training set: ", rmse(y_test, y_pred))
print('XGBoost model obtained rmse score of: %f' % (rmse_cv(model_xgb).mean()))

#dtrain = xgb.DMatrix(train_df_munged, label = label_df)
...
# Let the alphas be set automatically
model_en = linear_model.ElasticNetCV(l1_ratio=l1_ratio, max_iter=50000).fit(train_df_munged, y_train)
model_en.fit(train_df_munged, y_train)
print(model_en.l1_ratio_)
print('Elastic net model obtained rmse score of: %f' % (rmse_cv(model_en).mean()))
...
def cross_valid_kernel():
    ...
    best_alpha = None
    for alpha in alphas:
        model_kernel = KernelRidge(alpha=alpha, kernel='polynomial', degree=2, coef0=1.85)
        model_kernel.fit(train_df_munged, y_train)
        score = rmse_cv(model_kernel).mean()
        if score < min_score:
            min_score = score
            best_model = model_kernel
            best_alpha = alpha
    print('Kernel ridge model obtained rmse score of: %f' % (min_score))
    print('Best alpha for kernel ridge was: %f' % (best_alpha))
    return best_model

model_kernel = cross_valid_kernel()

#model_kernel = KernelRidge(kernel='polynomial', degree=2, coef0=1.85, alpha=0.0304, gamma=0.0001)
#model_kernel.fit(train_df_munged, y_train)
#print('Kernel ridge model obtained rmse score of: %f' % (rmse_cv(model_kernel).mean()))
#print(model_kernel.get_params())
...
# Run prediction of the Lasso model on the Kaggle test set.
y_pred_lasso = model_lasso.predict(test_df_munged)
y_pred_xgb = model_xgb.predict(test_df_munged)
y_pred_en = model_en.predict(test_df_munged)
y_pred_kernel = model_kernel.predict(test_df_munged)

#y_pred = (y_pred_xgb + y_pred_lasso + y_pred_en + y_pred_kernel) / 4
y_pred = (y_pred_kernel + y_pred_xgb) / 2
y_pred = np.exp(y_pred)

pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
pred_df.to_csv('output_kernel_xgb.csv', header=True, index_label='Id')
