...
class CustomEnsembleRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        for regressor in self.regressors:
            regressor.fit(X, y)
            print('Regression model obtained rmse score of: %f' % (rmse_cv(regressor,X,y).mean()))
    ...
    def predict(self, X):
        ...
        #print(self.predictions_[0])

        preds = self.predictions_[0]*.5+self.predictions_[1]*(0.5/3)+self.predictions_[2]*(0.5/3)+self.predictions_[3]*(0.5/3)

        #return np.log1p(np.mean(self.predictions_, axis=0))
        return np.log1p(preds)
...
regr3 = ElasticNet(alpha=0.001, max_iter=50000)
