...
regr = linear_model.Lasso(alpha=best_alpha, max_iter=50000)
regr.fit(train_df_munged, label_df)

# Run prediction on training set to get a rough idea of how well it does.
y_pred = regr.predict(train_df_munged)
print("Lasso score on training set: ", rmse(y_test, y_pred))
print('Lasso model obtained rmse score of: %f' % (rmse_cv(regr).mean()))
...
model_lasso = linear_model.LassoCV(alphas=alphas,max_iter=500000, cv=5).fit(train_df_munged, y_train)
y_test = label_df
y_pred = model_lasso.predict(train_df_munged)
# Run prediction on the Kaggle test set.
print("Lasso score on training set: ", rmse(y_test, y_pred))
...
y_pred = (y_pred_xgb + y_pred_kernel) / 2
y_pred = np.exp(y_pred)

y_list = [y_pred_xgb,y_pred_kernel]
y_pred_list = sum(y_list) / 2

if y_pred == y_pred_list:
    print('All gooooood')
else:
    print('shitttt')
